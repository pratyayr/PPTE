...
```{r}
attach(ebdf)
observation_date = order(observation_date)
for (i in 1:33275) {for (j in 1:33725)
  if ((observation_date[j]-40 <=  observation_date[i] <= observation_date[j]+40)&(common_name[i] = common_name[j]))
   {print(i)}
    else {print(i+1)}
  }
```
